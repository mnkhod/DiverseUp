...
describe("PausibleProxy Test", function () {
  var owner,odko,amaraa;
  var mockTokenContract;
  var pausibleProxyContract;
  ...
  beforeEach(async function () {
    ...
    const PausibleProxy = await ethers.getContractFactory("PausibleProxy");
    pausibleProxyContract = await PausibleProxy.deploy();
    await pausibleProxyContract.deployed();

    // Creating Pausible v2 Contracts
    let result = await ((await pausibleProxyContract.connect(odko).updateToken(
    ...
    newTokenContract = await ethers.getContractAt('PausibleToken',newTokenAddress) 

    expect(await newTokenContract.name()).to.equal("MockTokenV2");
    expect(await newTokenContract.symbol()).to.equal("MTNv2");

    expect(await newTokenContract.hasRole(PAUSER_ROLE,odko.address)).to.equal(true);
    expect(await exchangeContract.hasRole(ADMIN_ROLE,odko.address)).to.equal(true);

    expect(await exchangeContract.hasRole(ADMIN_ROLE,pausibleProxyContract.address)).to.equal(true);
    expect(await newTokenContract.hasRole(ADMIN_ROLE,pausibleProxyContract.address)).to.equal(true);
  });
