...
describe("VoteProxy Test", function () {
  var owner,odko,amaraa;
  var mockTokenContract;
  var voteProxyContract;
  ...
  beforeEach(async function () {
    ...
    voteProxyContract = await VoteProxy.deploy();
    await voteProxyContract.deployed();

    // Creating Vote v2 Contracts
    let result = await ((await voteProxyContract.connect(odko).updateToken(
    ...
    newTokenContract = await ethers.getContractAt('VoteToken',newTokenAddress) 
    ...
    expect(await exchangeContract.hasRole(ADMIN_ROLE,voteProxyContract.address)).to.equal(true);
    expect(await newTokenContract.hasRole(ADMIN_ROLE,voteProxyContract.address)).to.equal(true);
  });
